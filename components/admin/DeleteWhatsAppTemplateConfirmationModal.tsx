...
interface DeleteWhatsAppTemplateConfirmationModalProps {
    template: WhatsAppTemplate;
    onClose: () => void;
    showNotification: (message: string, type: NotificationType, duration?: number) => void;
}

const DeleteWhatsAppTemplateConfirmationModal: React.FC<DeleteWhatsAppTemplateConfirmationModalProps> = ({ template, onClose, showNotification }) => {
    
    const onConfirm = async () => {
        try {
            await db.collection("whatsApp_templates").doc(template.id).delete();
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[80] p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mt-4">Delete WhatsApp Template</h3>
                <div className="mt-2 text-sm text-gray-500">
                    <p>Are you sure you want to delete the template <strong className="font-bold">"{template.name}"</strong>?</p>
                    <p className="mt-2 text-xs text-red-600">This action cannot be undone. Any trigger assigned to this template will stop sending messages.</p>
                </div>
...
export default DeleteWhatsAppTemplateConfirmationModal;
